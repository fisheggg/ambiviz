...
class AmbiEncoder(object):
    ...
    def encode(self, sources):
        ...
        fmt = self.format
        Y = spherical_harmonics_matrix(
            [src.position for src in sources],
            fmt.order,
            fmt.ordering,
            fmt.normalization,
        )
    ...
    def encode_frame(self, sources, ambi_array, frame_no):
        ...
        src_signal = np.array([src.signal[frame_no] for src in sources]).reshape(
            (-1, 1)
        )
    ...
    def encode_v2(self, sources):
        ...
        for src in sources:
            if src.position.r > 0:
                ...
                delay = int(dist / 343.0 * src.sample_rate)
                attenuation = 1.0 / (1.0 + dist)
                src.signal = shift(src.signal, delay, cval=0.0) * attenuation
