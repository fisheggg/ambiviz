...
class CIPIC_HRIR(object):
    def __init__(self, dirname):
        ...
        for i, phi in enumerate(azimuth):
            ...
            self.right_hrir[:, i, :] = np.flip(
                load_wav(os.path.join(dirname, right_fn))[0], axis=0
            )
            self.left_hrir[:, i, :] = np.flip(
                load_wav(os.path.join(dirname, left_fn))[0], axis=0
            )
        ...
        for i, az in enumerate(azimuth):
            for j, elev in enumerate(elevation):
                xp = radius * cos(elev * pi / 180.0) * sin(az * pi / 180.0)
                yp = radius * cos(elev * pi / 180.0) * cos(az * pi / 180.0)
                zp = radius * sin(elev * pi / 180.0)
                x, y, z = yp, -xp, zp
                # x, y, z = xp, yp, zp
    ...
    def get_closest(self, pos):
        ...
        i = self.kdt.query(
            np.array([pos.x, pos.y, pos.z]).reshape((1, -1))
            / sqrt(pos.x**2 + pos.y**2 + pos.z**2)
        )[1][0, 0]
