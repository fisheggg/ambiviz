...
from .position import MovingSource, Position, PositionalSource
...
class VirtualStereoMic(object):
    ...
    def binauralize(self, sources):
        ...
        l_signal, r_signal = 0, 0.0
        for src in sources:
            l_dist = np.sqrt(
                (
                    (
                        src.position.coords("cartesian")
                        - self.lmic_pos.coords("cartesian")
                    )
                    ** 2
                ).sum()
            )
            r_dist = np.sqrt(
                (
                    (
                        src.position.coords("cartesian")
                        - self.rmic_pos.coords("cartesian")
                    )
                    ** 2
                ).sum()
            )
            ...
            # Time delay
            l_delay, r_delay = (
                int(l_dist / C * src.sample_rate),
                int(r_dist / C * src.sample_rate),
            )

            # Attenuation is frequency dependent, but lets simplify.
            l_attn, r_attn = 1 / (1.0 + l_dist), 1 / (1.0 + r_dist)

            l_signal += l_attn * shift(src.signal, l_delay, cval=0.0) / len(sources)
            r_signal += r_attn * shift(src.signal, r_delay, cval=0.0) / len(sources)
    ...
    def binauralize_frame(self, sources, output, frame_no):
        ...
        for src in sources:
            ...
            if frame_no - l_delay >= 0:
                output[frame_no, 0] += (
                    l_attn * src.signal[frame_no - l_delay] / len(sources)
                )
            if frame_no - r_delay >= 0:
                output[frame_no, 1] += (
                    r_attn * src.signal[frame_no - r_delay] / len(sources)
                )
...
class Convolvotron(object):
    ...
    def binauralize_frame(self, sources, output, frame_no):
        ...
        for src in sources:
            ...
            output[frame_no, 0] = (
                src.signal[i_start:i_end] * left_hrir[-i_range:]
            ).sum()
            output[frame_no, 1] = (
                src.signal[i_start:i_end] * right_hrir[-i_range:]
            ).sum()
...
class AmbisonicBinauralizer(object):
    def __init__(
        self, ambi_format, method="projection", use_hrtfs=False, cipic_dir=None
    ):
        ...
        if self.method == "pseudoinv":
            self.speaker_pos = map(
                lambda x: Position(x[0], x[1], x[2], "cartesian"),
                get_tDesign(self.fmt.order),
            )
            ...
            speakers_phi = (
                2.0
                * np.arange(2 * self.fmt.num_channels)
                / float(2 * self.fmt.num_channels)
                - 1.0
            ) * np.pi
            self.speaker_pos = map(
                lambda x: Position(x, 0, self.fmt.radius, "polar"), speakers_phi
            )
        else:
            raise ValueError(
    ...
    def binauralize(self, ambi):
        ...
        sources = map(
            lambda i: PositionalSource(
                speakers[:, i], self.speaker_pos[i], self.fmt.sample_rate
            ),
...
def test_convolvotron():
    ...
    positions = [
        [float(num) for num in l.strip().split()]
