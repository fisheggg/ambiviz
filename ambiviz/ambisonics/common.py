...
class AmbisonicArray(object):
    ...
    def convert(self, sample_rate=None, ordering=None, normalization=None):
        assert (
            sample_rate is not None or ordering is not None or normalization is not None
        )
        ...
        if ordering is not None and ordering != self.format.ordering:
            ...
            mapping = map(
                lambda x: convert_ordering(x, ordering, self.format.ordering), range(n)
            )
            data = data[:, mapping]
        ...
        if normalization is not None and normalization != self.format.normalization:
            ...
            c_out = np.array(
                map(
                    lambda x: normalization_factor(
                        x, self.format.ordering, normalization
                    ),
                    range(n),
                )
            )
            c_in = np.array(
                map(
                    lambda x: normalization_factor(
                        x, self.format.ordering, self.format.normalization
                    ),
                    range(n),
                )
            )
...
def degree_order_to_index(order, degree, ordering=DEFAULT_ORDERING):
    ...
    def sid_idx(n, m):
        idx_order = (
            ...
            + [n * 2]
            + list(reversed([i * 2 for i in range(n)]))
        )
    ...
    def fm_idx(n, m):
        if n == 1:
            ...
            idx_order = (
                list(reversed([2 * (i + 1) for i in range(n)]))
                + [0]
                + [1 + i * 2 for i in range(n)]
            )
        return idx_order[m + n] + n**2
...
def normalization_factor(
    index, ordering=DEFAULT_ORDERING, normalization=DEFAULT_NORMALIZATION
):
    assert ordering in CHANNEL_ORDERING
    assert normalization in NORMALIZATION
    ...
    def max_norm(n, m):
        ...
        if n == 0:
            ...
            return 1.0 if m == 0 else 2.0 / sqrt(3.0)
        else:
            return (
                1.0
                if m == 0
                else (sqrt(45.0 / 32) if m in [1, -1] else 3.0 / sqrt(5.0))
            )

    def sn3d_norm(n, m):
        return sqrt(
            (2.0 - float(m == 0))
            * float(factorial(n - abs(m)))
            / float(factorial(n + abs(m)))
        )

    def n3d_norm(n, m):
        return sn3d_norm(n, m) * sqrt((2 * n + 1) / (4.0 * pi))
...
def spherical_harmonic_mn(order, degree, phi, nu, normalization=DEFAULT_NORMALIZATION):
    ...
    norm = normalization_factor(
        degree_order_to_index(order, degree), normalization=normalization
    )
    sph = (
        (-1) ** degree
        * norm
...
def spherical_harmonics(
    position, max_order, ordering=DEFAULT_ORDERING, normalization=DEFAULT_NORMALIZATION
):
...
def spherical_harmonics_matrix(
    positions, max_order, ordering=DEFAULT_ORDERING, normalization=DEFAULT_NORMALIZATION
):
